...
async def scrape_noon_full(url: str) -> dict:
    ...
    rating = float(soup.find('div', class_='sc-e7071e85-2').get_text().strip())
    description = soup.find('div', class_='sc-97eb4126-2').get_text().strip()
    # Description sc-966c8510-2 
    d_local=soup.find('div', class_='sc-966c8510-2').get_text().strip()
    image_soup = soup.find_all('img')
    img=[]
    for i in image_soup:
        if "&width" in i['src']:
            img.append(i['src'])
    item_data = ProductDetailDTO(domainId=5,name_Global=g_name,name_Local=title, price=price, rating=rating, description_Global=description,images=img,productlink1=url,description_Local=d_local)
    return item_data
